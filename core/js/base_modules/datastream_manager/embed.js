...
function initDataService(){
    ...
    if (areEmbedOptionsEnabled) {
        ...
        var lName       = $fDataServiceContainer.data('dataservice_title') + ' | ' + $fDataServiceContainer.data('dataservice_subtitle');
...
function onSuccessDataServiceExecute(pResponse){
    ...
    var lDataserviceId = $fDataServiceContainer.data('dataservice_id');
    ...
    if(pResponse.fType!='ARRAY'){
        ...
        if(pResponse.fType == 'TEXT'){
            lValue = '<table class="Texto"><tr><td>' + pResponse.fStr + '</td></tr></table>';
        } else if(pResponse.fType == 'NUMBER'){
            lValue = '<table class="Numero"><tr><td>' + String(pResponse.fNum) + '</td></tr></table>';
        ...
        i = 0;
        lHtml  = lHtml + '<div class="dataStreamResult clearfix"><div class="Mensaje"><div id="id_fixedColumn" style="float:left;width:100px;"></div><div id="id_mainTable" style="float:left;overflow:auto;margin-top:-1px;"><table id="ladderTable" class="Tabla" >';
